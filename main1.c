...
void* simulate(void* j)
{
    int i;
    int thread = (int)j;
    int start = indexes[thread][0], end = indexes[thread][1];
    for (i = start; i < end; i++) {
...
{
    int i, j, first, last = 0;
    thread_count = strtol(argV[2], NULL, 10);
    pthread_t* thread_handles = malloc(thread_count * sizeof(pthread_t));
    indexes = (int**)malloc(thread_count * sizeof(int*));

    for (i = 0; i < thread_count; i++) {
        indexes[i] = (int*)malloc(thread_count * sizeof(int));
        indexes[i][0] = 0;
        indexes[i][1] = 0;
    }
    ...
    else
    {
        ...
        if (thread_count > bodies) {
            thread_count = bodies;
        }
        else {
            residual = bodies % thread_count;
        }

        for (long thread = 0; thread < thread_count; thread++)
        {
            first = last;
            indexes[thread][0] = first;
            if (residual > 0) {
                last = first + bodies / thread_count + 1;
                residual--;
            }
            else {
                last = first + bodies / thread_count;
            }
            indexes[thread][1] = last;
            printf("%d %d\n", first, last);
        }
        ...
        for (i = 0; i < timeSteps; i++)
        {
            for (long thread = 0; thread < thread_count; thread++)
            {
                pthread_create(&thread_handles[thread], NULL, simulate, (void*)(thread));
            }
